...
fn get_max_calories(filename: impl AsRef<Path>) -> i64 {
    ...
    iterate_numbers(filename, &mut|calorie_count| {
        match calorie_count {
...
pub fn part2_example_input() {
    let result = get_top_three_max_calories_sum("/workspaces/vscode-remote-try-rust/src/day1_example_input.txt");
    println!("{result}");
    assert!(result == 45000, "Expected 45000 but got {}", result);
}

pub fn part2_real_input() {
    let result = get_top_three_max_calories_sum("/workspaces/vscode-remote-try-rust/src/day1_input.txt");
    println!("{result}");
    //assert!(result == 45000, "Expected 45000 but got {}", result);
}